...
p_lower_bounds = [90, -2000, -2000, 10, 10, -100, -100, None, None]
p_upper_bounds = [270, 2000, 2000, 1000, 1000, 100, 100, None, None]
subplots = [list(plt.subplots(3, 1)) for _ in range(len(pnames))]
for i, (fig, *_) in enumerate(subplots):
    fig.suptitle(pnames[i])
    for ax in fig.axes:
        ax.set_ylim(p_lower_bounds[i], p_upper_bounds[i])
