...
def get_deconvolved_map_even(hp_map, beam_Bl, decimal_dec, decimal_ra, cmb_radius_deg, res, lmax, proj):
    ...
    enmap_deconvolved_cmb = deconvolution.get_deconvolved_map_odd(hp_map, beam_Bl, decimal_dec + half_res_deg, decimal_ra + half_res_deg, cmb_radius_deg, res=res, lmax=lmax, proj=proj)
...
def get_deconvolved_map(oshape, hp_map, beam_Bl, decimal_dec, decimal_ra, cmb_radius_deg, res, lmax, proj):
    ...
    if oshape[0] % 2 == 0:
        enmap_deconvolved_cmb = deconvolution.get_deconvolved_map_even(hp_map, beam_Bl, decimal_dec, decimal_ra, cmb_radius_deg, res=res, lmax=lmax, proj=proj)
    else:
        enmap_deconvolved_cmb = deconvolution.get_deconvolved_map_odd(hp_map, beam_Bl, decimal_dec, decimal_ra, cmb_radius_deg, res=res, lmax=lmax, proj=proj)
